...
import { Alimentacao, Decoracao, Midia, Musica, Musica2 } from 'assets';
          ...
          <div className="flex flex-col justify-between">
            <div className="flex gap-5 px-8 h-full w-full justify-center">
              {filteredServiceAlimentacao.map((serviceAlimentacao) => (
                <div
                  key={serviceAlimentacao.name}
                  className="flex flex-col justify-between pl-6 pr-6"
                >
                  <Link href={serviceAlimentacao.link}>
                    <Image
                      src={serviceAlimentacao.image}
                      alt={serviceAlimentacao.name}
                ...
                {filteredServiceMusica.map((serviceMusica) => (
                  ...
                  >
                    <Link href={serviceMusica.link}>
                      <Image
                        src={serviceMusica.image}
                        alt={serviceMusica.name}
                        className="w-220 h-auto"
                      />
                    </Link>
                  </div>
              ...
              <div className="justify-center">
                <div className="flex gap-5 px-8 h-full w-full justify-center">
                  {filteredServiceRest.map((serviceRest) => (
                    <div
                      key={serviceRest.name}
                      className="flex flex-col justify-between pl-6 pr-6"
                    >
                      <Link href={serviceRest.link}>
                        <Image
                          src={serviceRest.image}
                          alt={serviceRest.name}
                          className="w-220 h-auto"
                        />
                      </Link>
                    </div>
